...
class deformation():
    ...
    def get_random_displacement(self, amp=10):
        ...
        _, (u_1, v_1) = self.continuum_deformation(amp)
        ...
        return (u_pos + u_1 + u_4 + u_2, v_pos + v_1 + v_4 + v_2), (u_1 + u_2 + u_3 + u_4, v_1 + v_2 + v_3 + v_4), 1 - def_mask
    ...
    def get_random_displacement_no_crack(self, amp=10):
        ...
        # v_prev = v_1 + v_3 + v_4
        # (_, _, def_mask), (u_2, v_2) = self.crack_open_deformation(u_prev, v_prev)
        return (self.basic_u + u_1 + u_4, self.basic_v + v_1 + v_4), (u_1 + u_4, v_1 + v_4), np.ones_like(self.basic_u)

    def continuum_deformation(self, amp_r):
        amp = np.random.randint(2, 12) * amp_r / 8
        if np.random.randn() > 0.6:
            inter = 1
        else:
            inter = 2
        disp_seed_array_u = (np.random.random(size=(self.imsize[0]//np.random.randint(50, 200), self.imsize[1]//np.random.randint(50, 200))) - 0.5) * amp * 2
        disp_seed_array_v = (np.random.random(size=(self.imsize[0]//np.random.randint(50, 200), self.imsize[1]//np.random.randint(50, 200))) - 0.5) * amp * 2
        disp_u = cv2.resize(disp_seed_array_u, dsize=(self.imsize[1], self.imsize[0]), interpolation=inter)
        disp_v = cv2.resize(disp_seed_array_v, dsize=(self.imsize[1], self.imsize[0]), interpolation=inter)
    ...
    def crack_close_deformation(self):
        ...
        theta_0 = theta_0 / 10
        theta_1 = theta_1 / 10
        R = np.random.randint(100, self.imsize[0]//2)
        if R > 400:
    ...
    def affine_deformation(self, amp):
        rot_theta = np.random.randint(-2, 2) * amp / 100
        affine_ex = 1 + np.random.randint(-2, 2) * amp / 300
        affine_ey = 1 + np.random.randint(-2, 2) * amp / 300
        affine_shear_gx = np.random.randint(-1, 1) * amp / 100
        affine_shear_gy = np.random.randint(-1, 1) * amp / 100
        Tx = np.random.randint(-10, 10) * amp / 8
        Ty = np.random.randint(-10, 10)* amp / 8
